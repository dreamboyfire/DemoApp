/**
 * Created by s-guanhm on 2017/11/10.
 */
...
!function (a) {
  ...
  c.prototype.init = function () {
    ...
    if (this._currentIntervalStart = c.clone(), this.eventsLastMonth = [], this.eventsNextMonth = [], this.eventsThisInterval = [], this.options.events.length && (this.eventsThisInterval = a(this.options.events).filter(function () {
        ...
        return a || b ? !1 : !0
      }).toArray())), !this.options.lengthOfTime.days)if (i = l.weekday() - this.options.weekOffset, 0 > i && (i += 7), this.options.showAdjacentMonths)for (var m = 1; i >= m; m++) {
      var n = b([c.year(), c.month(), m]).subtract(i, "days");
      k.push(this.createDayObject(n, this.eventsLastMonth))
    } else for (var m = 0; i > m; m++)k.push(this.calendarDay({classes: this.options.targets.empty + " " + this.options.classes.lastMonth}));
    for (j = c.clone(); j.isBefore(d) || j.isSame(d, "day");)k.push(this.createDayObject(j.clone(), this.eventsThisInterval)), j.add(1, "days");
    if (!this.options.lengthOfTime.days)for (; k.length % 7 !== 0;)this.options.showAdjacentMonths ? k.push(this.createDayObject(j.clone(), this.eventsNextMonth)) : k.push(this.calendarDay({classes: this.options.targets.empty + " " + this.options.classes.nextMonth})), j.add(1, "days");
    if (this.options.forceSixRows && 42 !== k.length)for (; k.length < 42;)this.options.showAdjacentMonths ? (k.push(this.createDayObject(j.clone(), this.eventsNextMonth)), j.add(1, "days")) : k.push(this.calendarDay({classes: this.options.targets.empty + " " + this.options.classes.nextMonth}));
    return k
  }, c.prototype.createDayObject = function (a, c) {
    var d, e, f, g = 0, h = b(), i = [], j = "", k = {isToday: !1, isInactive: !1, isAdjacentMonth: !1};
    ...
    var a, c, d, e, f, g = {}, h = null, i = null, j = this.intervalEnd.clone().add(1, "years"),
    ...
    if (this.calendarContainer.empty(), this.options.lengthOfTime.days) a = this.createDaysObject(this.intervalStart.clone(), this.intervalEnd.clone()), g = {
      ...
      for (c = [], f = 0, e = [], o = 0; o < this.options.lengthOfTime.months; o++) {
        var l = this.intervalStart.clone().add(o, "months"), m = l.clone().endOf("month"),
          a = this.createDaysObject(l, m);
        e.push(this.eventsThisInterval), c.push({days: a, month: l})
      }
    ...
    var b = {}, c = this, d = a(this.element), e = this.options.targets, f = c.options.classes,
      g = this.options.useTouchEvents === !0 ? "touchstart" : "click", h = g + ".clndr";
    ...
    }), b = {context: this}, d.on(h, "." + e.todayButton, b, this.todayAction).on(h, "." + e.nextButton, b, this.forwardAction).on(h, "." + e.previousButton, b, this.backAction).on(h, "." + e.nextYearButton, b, this.nextYearAction).on(h, "." + e.previousYearButton, b, this.previousYearAction)
  }, c.prototype.buildTargetObject = function (c, d) {
    var e, f, g = {date: null, events: [], element: c};
    return d && (e = this.getTargetDateString(c), g.date = e ? b(e) : null, this.options.events && (f = this.options.multiDayEvents ? function () {
      var a = g.date.isSame(this._clndrStartDateObject, "day"), b = g.date.isAfter(this._clndrStartDateObject, "day"),
        c = g.date.isSame(this._clndrEndDateObject, "day"), d = g.date.isBefore(this._clndrEndDateObject, "day");
      return (a || b) && (c || d)
    } : function () {
    ...
    var d, e, f, g, h, i, j, k, l, m = a.options.lengthOfTime, n = a.options.clickEvents,
      o = {end: a.intervalEnd, start: a.intervalStart}, p = [b(a.intervalStart), b(a.intervalEnd)], q = [b(a.month)];
    ...
    var c = arguments.length > 1 ? arguments[1] : this, d = c.options.lengthOfTime, e = {withCallbacks: !1},
      f = {end: c.intervalEnd.clone(), start: c.intervalStart.clone()};
    ...
    a.forward({withCallbacks: !0}, a)
  }, c.prototype.next = function (a) {
    return this.forward(a)
  }, c.prototype.previousYear = function (b) {
    var c = arguments.length > 1 ? arguments[1] : this, d = {withCallbacks: !1},
      e = {end: c.intervalEnd.clone(), start: c.intervalStart.clone()};
    ...
    var b = a.data.context;
    b.previousYear({withCallbacks: !0}, b)
    ...
    var b = a.data.context;
    b.nextYear({withCallbacks: !0}, b)
  }, c.prototype.today = function (c) {
    var d = arguments.length > 1 ? arguments[1] : this, e = d.options.lengthOfTime, f = {withCallbacks: !1},
      g = {end: d.intervalEnd.clone(), start: d.intervalStart.clone()};
    ...
    var b = a.data.context;
    b.today({withCallbacks: !0}, b)
  }, c.prototype.setMonth = function (a, b) {
    var c = this.options.lengthOfTime, d = {end: this.intervalEnd.clone(), start: this.intervalStart.clone()};
    ...
    var c = {end: this.intervalEnd.clone(), start: this.intervalStart.clone()};
    ...
    var d = this.options.lengthOfTime, e = {end: this.intervalEnd.clone(), start: this.intervalStart.clone()};
    ...
    var c = {day: "", date: null, events: [], classes: this.options.targets.empty};
    return a.extend({}, c, b)
